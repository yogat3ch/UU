...
file_fn <- function(x, write = FALSE) {
  purrr::when(
    ...
    grepl("(?:png$)|(?:jpg$)|(?:jpeg$)", ., ignore.case = TRUE) && !write ~ purrr::when(UU::is_legit(utils::packageVersion("magick")), . ~ magick::image_read, ~ stop(x, " is an image and requires the magick package.")
    ),
