...
opts_helpers <- function(.Rprofile = ".Rprofile", .env = rlang::ns_env("UU")) {
  if (file.exists(.Rprofile) && is.null(get0("opts", envir = .env, inherits = FALSE))) {
    ...
    if (is_legit(calls)) {
      ...
      rp <- .Rprofile
      cli::cli_inform("{.pkg UU}: {.code opts} & {.code toggle} functions were generated for the following options in {.path {rp}}: {cli::col_br_blue(paste0(names(.opts), collapse = ', '))}", .frequency = "once")
    }
  }
}
